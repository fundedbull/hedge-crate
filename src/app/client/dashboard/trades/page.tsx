...
export default async function TradesPage({ searchParams }: { searchParams: Promise<{ year?: string, month?: string }> }) {
  ...
  const data = Array.from({ length: daysInMonth }, (_, i) => {
    const day = i + 1;
    return tradesByDay[day] || { day, trades: 0, amount: 0, performance: 'neutral' };
  });
  ...
  return (
    <div className="p-4">
      <TradingCalendarWrapper initialData={data} initialMonth={monthName} initialYear={year} initialTrades={trades} />
    </div>
  );
}
